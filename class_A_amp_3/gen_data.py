...
# X = [gain, bandwidth, VDD, Vin, Vto, THD=1]
# y = [R1, RD, Kp, CL]

os.system(f"rm {temp_path}*")
data = np.zeros((n_datapoints, 10))
...
def processing_data(raw_file, log_file, analysis_mode, i, VDD, Vin, R1, RD, CL, Vto, Kp):
    global data

    if analysis_mode == "TRAN":
        THD = float(getline(f'./{log_file}', 32)[29:-2])
        data[i, 2:] = [VDD, Vin, Vto, THD, R1, RD, Kp, CL]
    elif analysis_mode == "AC":
        read_log = LTSpiceLogReader(log_file)
        gain = np.abs(read_log.get_measure_value("a0"))
...
for i in range(n_datapoints):
    ...
    for key, value in params.items():
        if key == "Vto" or key == "Kp" or key == "Vin":
    ...
    runner.run(net, callback=processing_data, callback_args=(
        "TRAN", i, params["VDD"], params["Vin"], params["R1"], params["RD"], params["CL"], params["Vto"], params["Kp"]
    ))

    net.add_instruction('.ac dec 100 10 5k')
    runner.run(net, callback=processing_data, callback_args=(
        "AC", i, params["VDD"], params["Vin"], params["R1"], params["RD"], params["CL"], params["Vto"], params["Kp"]
    ))
