...
def train_model(model, train_loader, val_loader, optimizer, criterion, device, epochs=100, patience=10, plot=True):
    ...
    for epoch in range(epochs):
        ...
        for X_batch, y_batch in train_loader:
            ...
            #mag_pred = model.compute_mag_pred(y_pred)
            #loss = criterion(mag_pred, X_batch[:,0,:])
        ...
        with torch.no_grad():
            for X_batch, y_batch in val_loader:
                ...
                #mag_pred = model.compute_mag_pred(y_pred)
                #val_loss += criterion(mag_pred, X_batch[:,0,:]).item()
...
#'''
# --- Dataset & DataLoaders ---
full_dataset = LCDataset("data.npy")
#mag_max, mag_min, y_max, y_min = full_dataset.mag_max, full_dataset.mag_min, full_dataset.y_max, full_dataset.y_min
